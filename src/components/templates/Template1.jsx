...
const Template1 = ({
  ...
  return (
    ...
    >
        ...
        <div className="">
          ...
          {allDetailsFilled5 && (
            <div className="w-9 h-9 ps-3.5 pt-1 mt-5 bg-white rounded-2xl absolute top-96 left-9 font-bold">6</div>
          )}
          {allDetailsFilled6 && (
            <div className="w-9 h-9 ps-3.5 pt-1 mb-6 mt-7  bg-white rounded-2xl absolute top-80 left-9 font-bold">5</div>
          )}
        </div>
      )}
  ...
  <div key={index}>
    ...
    <ul className="flex text-xs sm:text-sm md:text-xs lg:text-xs m-2 gap-2">
      <li>{del.address}</li>
      <li className={`${del.phoneNumber ? 'before:content-["●"] before:m-2 font': ''} w-2/2 break-all`}>{del.phoneNumber}</li>
  ...
  {exp.companydescription.split(/\r?\n/).map((line, i) => (
    <li key={i} className={`${paragraphSpacingClass} ${line.trim() ? 'before:content-["•"] before:mr-1' : ''} text-xs sm:text-sm md:text-sm lg:text-sm m-2 w-2/2 break-all`}>
      {line}
    </li>
  ))}
</ul>
